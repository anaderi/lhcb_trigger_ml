...
class Predictions(object):
    ...
    def _compute_mse(self, var_names, target_efficiencies=None, stages=None, n_bins=20, power=2.):
        target_efficiencies = self._check_efficiencies(target_efficiencies)
        bin_indices = self._compute_bin_indices(var_names, n_bins=n_bins)
        compute_mse = lambda pred: computeMseVariationOnBins(pred, self.is_signal, bin_indices,
                                                             target_efficiencies=target_efficiencies, power=power)
    ...
    def print_mse(self, uniform_variables, efficiencies=None, stages=None, in_html=True):
        result = pandas.DataFrame(self._compute_mse(uniform_variables, efficiencies, stages=stages))
        if in_html:
    ...
    def efficiency(self, uniform_variables, stages=None, target_efficiencies=None, n_bins=20):
        target_efficiencies = self._check_efficiencies(target_efficiencies)
